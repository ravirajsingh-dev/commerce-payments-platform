...
export const requiredRule = (path, msg) => ({ path, msg });
...
export const emailRule = (path = "email") => ({
  path,
...
export const passwordMinRule = (path = "password", min = PASSWORD_MIN_LENGTH) => ({
  path,
  validator: (v) => String(v || "").trim().length >= min,
