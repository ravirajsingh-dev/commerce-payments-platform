...
import { createCarrier } from "@src/features/e-commerce/carriers/carrierActions";
...
const CreateCarrier = ({ createCarrier, setErrors, removeErrors, carriersStore }) => {
  ...
  const onSubmit = async (e) => {
    ...
    const result = await createCarrier(payload);
    if (result?.status) {
      removeErrors();
  ...
  return (
    <Container>
      ...
      <Card className="common-panel-card">
        <Card.Header>Create Carrier</Card.Header>
        <Card.Body>
          <form onSubmit={onSubmit}>
            ...
            <div className="d-flex justify-content-end gap-2 mt-3">
              ...
              >
                {loadingSubmit ? "Saving..." : "Create Carrier"}
              </Button>
            </div>
          </form>
        </Card.Body>
